...
const Nav = () => {
  ...
  return (
      <nav className='absolute border-2 border-sky_blue w-full'>
      <header className="p-4 dark:text-gray-100">
        <div className="container flex justify-between items-center h-16 mx-auto">
          ...
          <ul className={`items-stretch hidden text-[#fff] text-xl tracking-wide space-x-3 md:flex`}>
            {
              navManue.map(menu=> 
              <li className="flex">
          ...
          <button onClick={()=>setHamberger(!hamberger)} className="flex justify-end p-4 md:hidden">
